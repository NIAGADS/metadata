#! /usr/bin/env python3
"""DSS Metadata Validator

This script allows the user validate a sample or file manifest metadata file
arranged in tabular format (field names in columns, values in rows)
against a JSON-Schema file.

Results are piped to STDOUT.

This tool accepts delimited text files (.txt, .csv, .tab)  as well as excel
(.xls, .xlsx) files.
"""
import argparse
import json
...
if __name__ == "__main__":
    ...
    elif args.participantsOnly:
        result = validate_participant_info()
